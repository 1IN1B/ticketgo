...
export const userDb = {
  ...
  delete: (id) => {
    const transaction = db.transaction((userId) => {
      ...
      // Delete user's tickets (and comments on those tickets will cascade if enabled, but let's be safe)
      db.prepare("DELETE FROM tickets WHERE created_by = ?").run(userId);
      // Nullify assignments
      db.prepare(

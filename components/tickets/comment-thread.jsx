  ...
  return (
    <div className="relative space-y-6 before:absolute before:inset-0 before:ml-5 before:h-full before:w-0.5 before:bg-border before:content-['']">
      {comments.map((comment, index) => {
        ...
        const isOrgAdmin = comment.user_org_role === 'ORG_ADMIN';
        ...
        return (
          ...
          >
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-background z-10 border-2 border-border">
               <Avatar className="h-8 w-8">
                 <AvatarFallback className={cn(
                   "text-[10px]",
                   isOrgAdmin ? "bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-200" : "bg-sky-100 text-sky-700 dark:bg-sky-900 dark:text-sky-200"
                 )}>
                   {initials}
                 </AvatarFallback>
               </Avatar>
            ...
            <div className={cn(
              ...
              isOrgAdmin ? "bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100/50 dark:border-amber-900/30" : "bg-muted/30 border border-border/50",
